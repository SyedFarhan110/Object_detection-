 ...
 * 2. Model inference for face detection
 * 3. Post-processing of detected faces
...
class FaceDetectionHelper(
    ...
    companion object {
        ...
        private val FACE_COLOR = Color.parseColor("#00BFFF")  // Deep Sky Blue
    ...
    fun initialize(modelFile: File, labelsFile: File): Boolean {
        return try {
            Log.d(TAG, "Initializing Face Detection model...")
     ...
     */
    fun runInference(bitmap: Bitmap): List<FaceDetection> {
        ...
        try {
            ...
            Log.d(TAG, "✅ Detected ${nmsDetections.size} face(s)")
            nmsDetections.forEachIndexed { idx, det ->
                Log.d(TAG, "  Face ${idx + 1}: conf=${det.confidence}")
            }
    ...
    private fun postProcessOutput(
        ...
        for (i in 0 until NUM_ANCHORS) {
            ...
            val classId = 0
            val label = labels.getOrElse(classId) { "face" }
     ...
     * Calculate IoU
     */
    private fun calculateIoU(det1: FaceDetection, det2: FaceDetection): Float {
    ...
    fun drawDetections(
        ...
        for ((index, detection) in detections.withIndex()) {
            ...
            // Prepare label with face number
            val label = "Face ${index + 1} ${String.format("%.2f", detection.confidence)}"
